...
public class SlidingDistance
{
  ...
  //Constructor of SlidingDistance class
  public SlidingDistance(int[][] initial, int[][] goal, int sizeIn, PrintWriter PWOut)
  {
  ...
  private int heuristic(Board board)
  {
	  ...
	  for (int i = 0; i < size; i++)		//go thru board and
		for (int j = 0; j < size; j++)		//measure distance values
			if (board.array[i][j] != goal.array[i][j]) //if value mismatches goal board
			{
				...
				//find the city distance between two locations
